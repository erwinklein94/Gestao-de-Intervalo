...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!ownerProfile?.enabled || !ownerProfile.organization_member_id || !["editor", "coordinator"].includes(ownerProfile.role)) return json(origin, { error: "Link inválido ou indisponível." }, 404);
