...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: plan, error: planError } = await admin.from("interval_plans")
      .select("id,dataset_id,coordinator_member_id,client_id,title,service_type,contractor_name,foreman_name,coordinator,interval_date,location,window_start,window_end,planning_notes,execution_notes,is_locked,locked_at,status,completed_at,created_at,updated_at,front_position,front_name,weather,weather_note,weather_recorded_at,interval_steps(client_id,position,activity_name,planned_start,planned_end,actual_start,actual_end,actual_notes,status,skip_reason,updated_at)")
