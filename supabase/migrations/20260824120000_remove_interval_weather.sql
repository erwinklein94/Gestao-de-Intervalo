-- O clima sai do intervalo.
--
-- A pergunta "como esta o clima agora?" abria a execucao e nunca foi
-- respondida: nenhum dos intervalos gravados tem condicao, observacao ou
-- carimbo preenchido. Sem alimentacao, o campo nao vira indicador nem explica
-- desvio -- so ocupa o topo da tela que o coordenador usa com pressa, no
-- momento em que ele quer registrar horario.
--
-- Nao ha dado a preservar: a verificacao antes desta migracao encontrou zero
-- registros nas tres colunas.
--
-- sync_interval_plan e reescrita a partir da versao em producao apenas para
-- deixar de gravar as colunas. Corpo de plpgsql nao e verificado no drop, e
-- sem esta reescrita a sincronizacao quebraria no primeiro salvamento.

-- Sincronizacao sem as colunas de clima.
...
-- As restricoes caem junto com as colunas que sustentam.
