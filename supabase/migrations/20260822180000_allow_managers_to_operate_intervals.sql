-- Gerentes passam a criar, executar, comentar, compartilhar e finalizar os
-- próprios intervalos, sem perder a leitura dos intervalos da equipe.
    ...
    and case private.current_role()
      ...
      when 'executive_manager' then
        exists (
          ...
          where manager.id = target_coordinator_id
            and manager.dataset_id = target_dataset_id
      ...
      when 'manager' then
        target_coordinator_id = private.current_member_id()
        or exists (
          select 1
          from public.manager_operator_assignments assignment
          where assignment.dataset_id = target_dataset_id
...
begin
  ...
  if coalesce(private.actual_role() not in ('manager', 'coordinator', 'specialist'), true) then
    raise exception 'Somente Gerente, Coordenador, Coordenadora ou Especialista pode finalizar a execucao.' using errcode = '42501';
  end if;
